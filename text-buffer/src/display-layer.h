...
struct DisplayLayer {
  ...
  Point translateBufferPositionWithSpatialIndex(const Point &, ClipDirection = ClipDirection::closest);
  Range translateBufferRange(const Range &, ClipDirection = ClipDirection::closest);
  Point translateScreenPosition(Point, ClipDirection = ClipDirection::closest, bool = false);
  Point translateScreenPositionWithSpatialIndex(const Point &, ClipDirection = ClipDirection::forward, bool = false);
  Range translateScreenRange(const Range &, ClipDirection = ClipDirection::closest, bool = false);
  Point clipScreenPosition(const Point &);
  Point constrainScreenPosition(const Point &, ClipDirection);
  Point expandHardTabs(const Point &, const Point &, double);
  Point collapseHardTabs(const Point &, double, ClipDirection);
  double getClipColumnDelta(const Point &, ClipDirection);
  ...
  int32_t scopeIdForTag(int32_t);
  std::string classNameForTag(int32_t);
  int32_t openTagForScopeId(int32_t);
  int32_t closeTagForScopeId(int32_t);
  bool isOpenTag(int32_t);
  bool isCloseTag(int32_t);
  void bufferWillChange(const Range &);
  void bufferDidChange(const Range &, const Range &);
